...
def no_perturbation(tar_pdb, res_phi_chem, atom_dict,
    ...
    for i in range(0, len(mut_info)):
        ...
        if mut_chain not in c_id.keys():
            continue

        for res in pdb_model[mut_chain]:
            res_idx = str(res.get_id()[1]) + res.get_id()[2].lower()
            res_idx = res_idx.strip()
            res_name = res.get_resname()
            cr_token = '{}_{}'.format(mut_chain, res_idx)
        ...
        for ir in interface_res:
            ...
            if ir_chain  !=  mut_chain:
                ir_remove.append(ir)
        for ir in ir_remove:
            interface_res.remove(ir)
        for _ in range(1):
            if mut_info[i][1]+ '_' + mut_info[i][2] not in chain_dict.keys():
                mut_isava = False

                break
            mut_list = choose_mut_residue(chain_dict, mut_info[i][1] + '_' + mut_info[i][2])
            for e in mut_list:
        ...
        index = 'piano/Data/apo_single1' + '/' + name_key + '_A_' + 'index_dict.pt'
        torch.save(res_atom_num, index)
        if len(p_features_atom) == 0 or len(p_features_res) == 0:
            return False
        gb_pos_64(p_features_atom, p_features_res, 'piano/Data/apo_single1', name_key+'_A')
...
def main():
    ...
    for i in range(0, len(key_l)):
        ...
        print(pdb + '_' + str(i))
        flag = no_perturbation(pdb.lower(), res_phi_chem, atom_dict, residues, res_code, res_corr, pre_ans[2][0], had_pdb, key)
        print(flag)

...
def interfaceResidues(cmpx, cA='c. A', cB='c. B', cutoff=1.0, selName="interface"):
	...
	# operate on a new object & turn off the original 创建一个复合物的temp
	cmd.create(tempC, cmpx)
	cmd.disable(cmpx)
	
	# remove cruft and inrrelevant chains 移除相关的原子
	cmd.remove(tempC + " and not (polymer and (%s or %s))"	 % (cA, cB))
	
	# get the area of the complete complex 获取选取的总面积，off-计算溶剂排除曲面面积
	...
	txtfile = 'temp/temp_{}.txt'.format(cmpx)
